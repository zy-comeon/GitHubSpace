...
public class LoginController extends BaseController {
	...
	public Object login()throws Exception{
		...
		pd = this.getPageData();
		String errInfo = "";
		String KEYDATA[] = pd.getString("KEYDATA").replaceAll("qfh", "").replaceAll("QQ978336446fh", "").split(",fh,");
		if(null != KEYDATA && KEYDATA.length == 3){
			...
			if(null == code || "".equals(code)){//判断效验码
				...
				if(true){//Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)		//判断登录验证码
	...
	public PageData setLoginPd(PageData pd){
		...
		if(null != strLOGINEDIT && !"".equals(strLOGINEDIT)){
			...
			if(strLo.length == 2){
				pd.put("isZhuce", strLo[0]);//是否可注册
				pd.put("isMusic", strLo[1]);//是否有音乐
			}
		}
		try {
